...
const IFarmStrategy = artifacts.require("YelHodlStrategyMainnet_iFARM");
...
describe("Mainnet iFARM YEL HODL", function() {
  let accounts;
  ...
  // external setup
  let underlyingWhale = "0x7e118a15C43B8F56Cf69a4536FDd2404aC457552";
  ...
  async function setupExternalContracts() {
    underlying = await IERC20.at("0x1571eD0bed4D987fe2b498DdBaE7DFA19519F651");
  ...
  before(async function() {
    ...
    [controller, vault, strategy, potPool] = await setupCoreProtocol({
      "existingVaultAddress": null,
      "strategyArtifact": IFarmStrategy,
      "strategyArtifactIsUpgradable": true,
  ...
  describe("Happy path", function() {
    it("Farmer should earn money", async function() {
      ...
      let ifarmVault = await IVault.at(underlying.address);
      ...
      for (let i = 0; i < hours; i++) {
        ...
        lpPrice = (new BigNumber(await priceOracle.getPrice(addresses.FARM))).times(new BigNumber(await ifarmVault.getPricePerFullShare())).div(D18);
