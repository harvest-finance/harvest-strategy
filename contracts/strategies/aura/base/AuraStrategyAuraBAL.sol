...
import "../interface/IAuraDepositor.sol";
import "../../../base/interface/IStrategy.sol";
import "../../../base/interface/IVault.sol";
import "../../../base/interface/balancer/IBVault.sol";

contract AuraStrategyAuraBAL is 
  IStrategy,
  BaseUpgradeableStrategyUL
{
  ...
  bytes32 internal constant _SWAP_NTOKENS_SLOT = 0x75d9186c8568828f230a038dfdca1c1ab775e3f419ccc8600fb5c1175bc12737;
  bytes32 internal constant _DEPOSIT_NTOKENS_SLOT = 0xe1e53e38178c2ec3bbd8e51caede18294e33412d8b51b83a61c9d1e93a69e6a7;
  ...
  constructor() public BaseUpgradeableStrategyUL() {
    ...
    assert(_SWAP_NTOKENS_SLOT == bytes32(uint256(keccak256("eip1967.strategyStorage.swapNTokens")) - 1)); 
    assert(_DEPOSIT_NTOKENS_SLOT == bytes32(uint256(keccak256("eip1967.strategyStorage.depositNTokens")) - 1)); 
  }
  ...
  function initializeBaseStrategy(
    ...
    address _lpt = IAuraBaseRewardPool(_rewardPool).stakingToken();
  ...
  function _setAuraDeposit(address _address) 
    internal 
  {
  ...
  function _setSwapNTokens(uint256 _value) internal {
    setUint256(_SWAP_NTOKENS_SLOT, _value);
  }

  function _setDepositNTokens(uint256 _value) internal {
    setUint256(_DEPOSIT_NTOKENS_SLOT, _value);
  }
  ...
  function _setDepositArrayIndex(uint256 _value) 
    internal 
  {
    require(_value <= depositNTokens(), "Invalid index");
  ...
  function swapNTokens() 
    ...
    returns (uint256) 
  {
    return getUint256(_SWAP_NTOKENS_SLOT);
  }

  function depositNTokens() 
    public 
    view 
    returns (uint256)
  {
    return getUint256(_DEPOSIT_NTOKENS_SLOT);
  }
  ...
  function _liquidateReward() 
    internal 
  {
    // Swap BAL into 80BAL20WETH
    // Deposit 80BAL20WETH into Aura: auraBALBpt Depositor
    // Stake all auraBALBpt
    ...
    for(uint256 i = 0; i < rewardTokens.length; i++){
      ...
      if(token == universalRewardToken) {
        ...
        continue;
      } else if (token == bbaUSD) {
        _liquidateBbaUSD();
        continue;
      } else if (storedLiquidationDexes[token][universalRewardToken].length < 1) {
        continue;
      }
    ...
    _convertCrvToAuraBAL();
  }

  function _liquidateBbaUSD()
    internal
  {
    address payable recipient = address(uint160(address(this)));
    uint256 batchSwapTokens = swapNTokens();
    uint256 tokenBalance = IERC20(bbaUSD).balanceOf(address(this));

    IERC20(bbaUSD).safeApprove(bVault, 0);
    IERC20(bbaUSD).safeApprove(bVault, tokenBalance);
  ...
  // Reward => Deposit(WETH) => JoinPool Bpt => Deposit(Bpt) => StakeAll
  function _enterRewardPool() 
    internal 
  {
    address lpToken = underlying();
    address stakePool = rewardPool();
    uint256 entireBalance = IERC20(lpToken).balanceOf(address(this));
    IERC20(lpToken).safeApprove(stakePool, 0);
    IERC20(lpToken).safeApprove(stakePool, entireBalance);
