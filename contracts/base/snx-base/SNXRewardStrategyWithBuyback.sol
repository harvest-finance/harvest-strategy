...
contract SNXRewardStrategyWithBuyback is StrategyBase {
  ...
  */
  ...
  function _liquidateReward() internal {
    ...
    notifyProfitAndBuybackInRewardToken(rewardBalance, distributionPool, buybackRatio);
    ...
    // approve uniswap router to sell reward
    ...
    if (uniswapRoutes[uniLPComponentToken0].length > 1) {
      ...
      // else if token0 is the reward token
    ...
    if (uniswapRoutes[uniLPComponentToken1].length > 1) {
      ...
      // else if token1 is the reward token
      token1Amount = toToken1;
    }

    // provide token1 and token2 to uniswap
