...
contract VaultV3 is ERC20, ERC20Detailed, IUpgradeSource, ControllableInit, VaultStorage {
  ...
  */
  function doHardWork() whenStrategyDefined onlyControllerOrGovernance external {
    ...
    invest();
    IStrategyV3(strategy()).doHardWork();
  }
  ...
  */
  function underlyingBalanceWithInvestment() view public returns (uint256) {
    ...
    return underlyingBalanceInVault().add(IStrategyV3(strategy()).investedUnderlyingBalance());
  }
  ...
  */
  ...
  function setStrategy(address _strategy) public onlyControllerOrGovernance {
    ...
    require(IStrategyV3(_strategy).underlying() == address(underlying()), "Vault underlying must match Strategy underlying");
    require(IStrategyV3(_strategy).vault() == address(this), "the strategy does not belong to this vault");
    ...
    if (address(_strategy) != address(strategy())) {
      if (address(strategy()) != address(0)) { // if the original strategy (no underscore) is defined
        ...
        IStrategyV3(strategy()).withdrawAllToVault();
      }
  ...
  function availableToInvestOut() public view returns (uint256) {
    ...
    uint256 alreadyInvested = IStrategyV3(strategy()).investedUnderlyingBalance();
  ...
  */
  ...
  function withdraw(uint256 numberOfShares) external {
    ...
    if (underlyingAmountToWithdraw > underlyingBalanceInVault()) {
      ...
      if (numberOfShares == totalShareSupply) {
        ...
        IStrategyV3(strategy()).withdrawToVault(missingShares, (totalSupply()).add(missingShares));
        underlyingAmountToWithdraw = underlyingBalanceInVault();
      }
    }
  ...
  function _deposit(uint256 amount, address sender, address beneficiary) internal {
    ...
    if (address(strategy()) != address(0)) {
      require(IStrategyV3(strategy()).depositArbCheck(), "Too much arb");
    }
