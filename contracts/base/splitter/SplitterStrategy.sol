...
import "../interface/IStrategyV3.sol";
...
contract SplitterStrategy is IStrategyV3, Initializable, ControllableInit, SplitterStorage {
  ...
  */
  ...
  function initSplitter(
    ...
    _setNextImplementationDelay(splitterUpgradeTimelock);
  }
  ...
  */
  ...
  function depositArbCheck() public view returns (bool) {
    for (uint256 i = 0; i < activeStrategies.length; i++) {
      if (!IStrategyV3(activeStrategies[i]).depositArbCheck()) {
        return false;
      }
    }
    return true;
  ...
  */
  function investedUnderlyingBalance() public view returns (uint256) {
    uint256 result = 0;
    for (uint256 i = 0; i < activeStrategies.length; i++) {
      result = result.add(IStrategyV3(activeStrategies[i]).investedUnderlyingBalance());
    }
  ...
  */
  function withdrawFromStrategy(address strategy, uint256 correspondingShares, uint256 totalShares) public onlyGovernance {
    ...
    require(correspondingShares > 0, "cannot withdraw 0");
    IStrategyV3(strategy).withdrawToVault(correspondingShares, totalShares);
  }
  ...
  */
  function moveAllAcrossStrategies(address sourceStrategy, address destinationStrategy) external onlyGovernance {
    ...
    uint256 balanceBefore = IERC20(underlying()).balanceOf(address(this));
    IStrategyV3(sourceStrategy).withdrawAllToVault();
    uint256 amount = IERC20(underlying()).balanceOf(address(this)).sub(balanceBefore);
    require(amount > 0, "resulting amount must be greater than 0");
    investIntoStrategy(destinationStrategy, amount);
  }
  ...
  */
  function withdrawAllToVault() external restricted {
    for (uint256 i = 0; i < activeStrategies.length; i++) {
      if (IStrategyV3(activeStrategies[i]).investedUnderlyingBalance() > 0) {
        IStrategyV3(activeStrategies[i]).withdrawAllToVault();
      }
    }
  ...
  */
  function withdrawToVault(uint256 correspondingShares, uint256 totalShares) external restricted {
    ...
    for (uint256 i = 0; i < activeStrategies.length; i++) {
      uint256 strategyBalance = IStrategyV3(activeStrategies[i]).investedUnderlyingBalance();
      if (strategyBalance > 0) {
        IStrategyV3(activeStrategies[i]).withdrawToVault(
  ...
  */
  function doHardWork() public restricted {
    investAllUnderlying();
    for (uint256 i = 0; i < activeStrategies.length; i++) {
      IStrategyV3(activeStrategies[i]).doHardWork();
    }
  }
  ...
  */
  function doHardWork(address _strategy) public restricted {
    IStrategyV3(_strategy).doHardWork();
  }
  ...
  */
  function announceStrategyWhitelist(address _strategy) public onlyGovernance {
    ...
    require(IStrategyV3(_strategy).underlying() == address(underlying()), "Underlying of splitter must match Strategy underlying");
    require(IStrategyV3(_strategy).vault() == address(this), "The strategy does not belong to this splitter");
  ...
  */
  function unwhitelistStrategy(address _strategy) public onlyGovernance {
    ...
    IStrategyV3 strategy = IStrategyV3(_strategy);
